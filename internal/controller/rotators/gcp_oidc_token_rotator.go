...
func NewGCPOIDCTokenRotator(
	...
	// Validate required GCP configuration parameters
	if bsp.Spec.GCPCredentials == nil {
		return nil, fmt.Errorf("invalid backend security policy, gcp credentials cannot be nil")
	}

	gcpCreds := bsp.Spec.GCPCredentials
	if gcpCreds.ProjectName == "" {
		return nil, fmt.Errorf("invalid GCP credentials configuration: projectName cannot be empty")
	}
	if gcpCreds.Region == "" {
		return nil, fmt.Errorf("invalid GCP credentials configuration: region cannot be empty")
	}

	wifConfig := gcpCreds.WorkLoadIdentityFederationConfig
	if wifConfig.ProjectID == "" {
		return nil, fmt.Errorf("invalid GCP Workload Identity Federation configuration: projectID cannot be empty")
	}
	if wifConfig.WorkloadIdentityPoolName == "" {
		return nil, fmt.Errorf("invalid GCP Workload Identity Federation configuration: workloadIdentityPoolName cannot be empty")
	}
	if wifConfig.WorkloadIdentityProvider.Name == "" {
		return nil, fmt.Errorf("invalid GCP Workload Identity Federation configuration: workloadIdentityProvider.name cannot be empty")
	}
...
func (r *gcpOIDCTokenRotator) Rotate(ctx context.Context) (time.Time, error) {
	...
	if r.gcpCredentials.WorkLoadIdentityFederationConfig.ServiceAccountImpersonation != nil {
		gcpAccessToken, err = r.impersonateServiceAccount(ctx, stsToken.Token)
		if err != nil {
	...
	if err != nil {
		if apierrors.IsNotFound(err) {
			...
			populateAccessTokenInSecret(secret, gcpAccessToken, GCPAccessTokenKey)
...
// This is a key step in the GCP Workload Identity Federation flow:
//  1. The JWT (JSON Web Token) from an external identity provider (like a Kubernetes ServiceAccount)
//     is sent to GCP's Security Token Service
//  2. GCP validates the token against the configured Workload Identity Provider
//  3. If valid, GCP returns an STS token that can be used for GCP API authentication
//
// The audience for the token exchange is constructed using the configured:
// - GCP Project ID
// - Workload Identity Pool name
// - Workload Identity Provider name
//
// The resulting STS token has the IAM scope (https://www.googleapis.com/auth/iam),
// which allows it to be used for service account impersonation.
//
// Returns the STS access token or an error if the exchange fails.
func (r *gcpOIDCTokenRotator) exchangeJWTForSTSToken(ctx context.Context, jwtToken string, opts ...option.ClientOption) (*tokenprovider.TokenExpiry, error) {
...
// This method takes an STS token and uses it to impersonate a GCP service account,
// generating a new access token with the permissions of that service account.
//
// The service account email is constructed from the serviceAccountName and serviceAccountProjectName
...
func (r *gcpOIDCTokenRotator) impersonateServiceAccount(ctx context.Context, stsToken string, opts ...option.ClientOption) (*tokenprovider.TokenExpiry, error) {
